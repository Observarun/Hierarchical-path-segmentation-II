...
```
such that each segment has $`\mu`$ points. The segmentation can be performed in two ways: as a vector (or feature) representation using means, standard deviations, and net displacement[[2]](#2); and as multivariate time-series of $`\mu`$ points. These segments are then categorised using a clustering algorithm into $`n`$ clusters. The cluster centroid, which is a representative segment for a cluster (e.g., mean of all segments belonging to that cluster), is called a StaME:
```math
...
The algorithms I use for clustering our time-series data can be classified as feature-based and shape-based. In feature-based (also termed vector clustering) approaches, a feature vector representation is contructed from the multivariate (speed and turning angle) time-series (base segments or words as explained above). This representation has statistical summary variables (means, standard deviations) and net displacement as in [[1]](#1). I explored several classic ML and DL algorithms for unsupervised classification - including DTC: Deep Temporal Clustering (Naveen Sai Madiraju et al., 2018-02), ISVAE: Interpretable Spectral Variational AutoEncoder (Jimenez Rama et al., 2023-10), DPSOM: Deep Probabilistic Clustering with Self-Organizing Maps (Laura Manduchi et al., 2019-10), SLAC-Time: Self-supervised Learning-based Approach to Clustering multivariate Time-series data (Hamid Ghaderi et al., 2023-02) - and used the following vector clustering approaches.
