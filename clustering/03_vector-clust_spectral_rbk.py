...
df_chunks_DARs = df_chunks_DARs[None]
df_chunks_DARs = df_chunks_DARs.drop('seg', axis=1)


arr_chunks_DARs = df_chunks_DARs.to_numpy()

norm_arr_chunks_DARs = np.sum(arr_chunks_DARs**2, axis=-1)

gaussian_kernel = ne.evaluate(
  'exp(-g * (A + B - 2 * C))',
  {'A' : norm_arr_chunks_DARs[:,None], 'B' : norm_arr_chunks_DARs[None,:], 'C' : np.dot(arr_chunks_DARs, arr_chunks_DARs.T), 'g' : 0.5}
...
for ic in range(0,nk):
    ...
    for idx in np.where(result_clustering==ic):
        speed_mean.extend(df_chunks_DARs.loc[idx,'speed_mean'])
        speed_std.extend(df_chunks_DARs.loc[idx,'speed_std'])
        turning_angle_mean.extend(df_chunks_DARs.loc[idx,'turning_angle_mean'])
        turning_angle_std.extend(df_chunks_DARs.loc[idx,'turning_angle_std'])
        net_displacement.extend(df_chunks_DARs.loc[idx,'net_displacement'])
    ...
    cluster_statistics_occupancy.append([sum(speed_mean)/len(speed_mean), sum(speed_std)/len(speed_std), sum(turning_angle_mean)/len(turning_angle_mean), sum(turning_angle_std)/len(turning_angle_std), sum(net_displacement)/len(net_displacement), len(speed_mean)])
...
result_clustering_new_order = np.array(
  result_clustering_new_order,
  dtype='int'
)  # clustering sequence with new cluster order
np.savetxt(
